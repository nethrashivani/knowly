...
export default function WorkshopsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return workshops.filter((w) => [w.title, w.description, w.teacherName, w.location, w.roomName].some((v) => v?.toLowerCase().includes(q)));
  }, [workshops, search]);

  const handleApply = async (id) => {
    if (applying === id || appliedIds.has(id)) return;
    try { setApplying(id); setError(''); await applyForWorkshop(id); setAppliedIds((prev) => new Set(prev).add(id)); }
  ...
  const handleDelete = async (workshop) => {
    ...
    try {
      setDeleting(workshop.id); setError('');
      await deleteWorkshop(workshop.id);
      setWorkshops((prev) => prev.filter((item) => item.id !== workshop.id));
    } catch (err) { setError(err.response?.data?.message || 'Failed to delete workshop.'); }
