...
export default function RoomPage() {
  ...
  const loadRooms = async () => {
    try { setLoading(true); setRooms(await getMyRooms()); setError(''); }
  ...
  const handleCreate = async (e) => {
    ...
    try { setSaving(true); setError(''); const r = await createRoom(roomName.trim()); setRoomName(''); setMode(null); setMessage(`Created "${r.name}". You are now inside this room.`); await loadRooms(); }
    catch (err) { setError(err.response?.data?.message || 'Failed to create room.'); }
    finally { setSaving(false); }
  };
  const handleJoin = async (e) => {
    e.preventDefault(); if (!code.trim()) return;
    try { setSaving(true); setError(''); const r = await joinRoom(code.trim().toUpperCase()); setCode(''); setMode(null); setMessage(`Joined "${r.name}". It is now your active room.`); await loadRooms(); }
  ...
  const activeRoom = rooms.find(r => r.active);

  return <><Navbar /><div className="min-h-screen bg-gray-50 px-4 py-8"><div className="max-w-5xl mx-auto">
    <button onClick={() => navigate('/')} className="text-blue-600 hover:text-blue-800 font-medium mb-6">← Back to Home</button>
    <div className="mb-7"><p className="text-blue-600 font-semibold">PRIVATE LEARNING SPACE</p><h1 className="text-3xl font-bold text-gray-900 mt-1">Your Rooms</h1><p className="text-gray-600 mt-2">Join multiple organization rooms and switch between them. Each room has its own workshops and learning space.</p></div>
    ...
    <div className="flex flex-wrap gap-3 mb-6"><button onClick={() => setMode('create')} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-blue-700">+ Create a Room</button><button onClick={() => setMode('join')} className="bg-gray-900 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-black">Join a Room</button>{activeRoom && <button onClick={() => navigate('/workshops/create')} className="border border-blue-200 bg-white text-blue-600 px-5 py-2.5 rounded-lg font-medium hover:bg-blue-50">+ Create Workshop in {activeRoom.name}</button>}</div>
